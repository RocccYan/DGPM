...
from MotiFiesta.pretrain.utils import load_saved_model, set_device
from MotiFiesta.src.loading import get_loader,get_loader_motif_pred


class Decoder:
    def __init__(self, model_id, dataset_id, load=True, model_file_name=None):
        ...
        print(self.model)
        self.dataset = get_loader_motif_pred(name=dataset_id) if load else dataset_id
        pass
...
class HashDecoder(Decoder):
    def __init__(self, model_id, dataset_id, hash_dim=4, dummy=False, level=2, load=True, model_file_name=None):
        self.level = level
        self.dummy = dummy
    ...
    def decode(self, n_graphs=-1):
        ...
        for idx, g in enumerate(self.dataset):
            ...
            batch = torch.zeros(len(g.x), dtype=torch.long).cuda()
            ...
            # roc, include all graoh to be assigned
            ...
            level = min(len(embs)-1, self.level)
            for i,x in enumerate(embs[level]):
                # print('*'*20)
                hash_table.index(x.cpu().detach().numpy())
                h = hash_table.query(x.cpu().detach().numpy())[0]
                # def total_sigma(self, level, node, tree, sigmas, ee):
                # spotlight = list(merge_info['spotlights'][self.level][i])
                spotlight = list(merge_info['spotlights'][level][i])
                # score = self.total_sigma(self.level, i, merge_info['tree'], probas, ee)
                score = self.total_sigma(level, i, merge_info['tree'], probas, ee)
                hash_set.add(h)
