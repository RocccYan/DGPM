...
class EdgePooling(torch.nn.Module):
    ...
    def forward(self, x, edge_index, batch, hard_embed=False, dummy=False):
        ...
        # logging.info(f"EdgePool: Before __merge_edges__ {time.time()}")
        x_new, edge_index, batch, unpool_info = self.__merge_edges__(
            x, edge_index, batch, e, x_merged, x_merged_self)
        

        # TODO
        # logging.info(f"EdgePool: Before return {time.time()}")
    ...
    def __merge_edges__(self, x, edge_index, batch, edge_score, x_merged, x_merged_self):
        # logging.info(f"EdgePool: __merge_edges__ start {time.time()}")
        nodes_remaining = set(range(x.size(0)))

        # cluster = torch.empty_like(batch, device=torch.device('cpu'))
        cluster = torch.empty_like(batch, device=x.device)

        # logging.info(f"EdgePool: __merge_edges__ Before sort {time.time()}")
        ...
        # logging.info(f"EdgePool: __merge_edges__ Before edge_iteration {time.time()}")
        ...
        # logging.info(f"EdgePool: __merge_edges__ Before new_x {time.time()}")
        # carlos
        ...
        # print(new_x[:5,:5])
        # exit()
        # logging.info(f"EdgePool: __merge_edges__ Before new_edge_score {time.time()}")
        ...
        # logging.info(f"EdgePool: __merge_edges__ Before new_edge_index {time.time()}")
        N = new_x.size(0)
        ...
        # logging.info(f"EdgePool: __merge_edges__ Before new_batch {time.time()}")
        new_batch = x.new_empty(new_x.size(0), dtype=torch.long)
        new_batch = new_batch.scatter_(0, cluster, batch)

        # i added e to the output (edge scores for original graph)
        # logging.info(f"EdgePool: __merge_edges__ Before unpool {time.time()}")
