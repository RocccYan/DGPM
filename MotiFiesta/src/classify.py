...
if __name__ == "__main__":
    # d = 'NCI1'
    # d = ["PROTEINS", "ENZYMES", "COX2"]
    for data in ["PROTEINS", "IMDB-BINARY", "COX2"]:
        
        clf = MotiFiestaClassifierSK(data, dummy=False, sigma_filter='all')
        loaders = get_loader(d, batch_size=8)
        # acc = clf.eval_kfold(loaders['dataset_whole'], f'supervised_data/fold-idx/{d}')
        acc = clf.eval_kfold(loaders['dataset_whole'], f'supervised_data/fold-idx/TUDataset/{data}')
        print(acc)
        # pass
