...
class MotiFiesta(torch.nn.Module):
    ...
    def rec_loss(self,
        ...
        def get_edge_subgraphs_by_level(level, ee, internals, spotlights,graphs, node_feats, batch):
            ...
            # tt_matrix_cosine = time.time()
            # print(f"Extract time: matrix_cosine: {tt_matrix_cosine - tt_get_edge_sg}")
            # K_predict = K_predict.to(get_device())
            # wwl_start = time.time()
            # extract_time = wwl_start - extract_start
            # print(f"Extract time: {extract_time}")
            K_true = build_wwl_K(subgraphs[:num_nodes], node_features[:num_nodes])
            # build_time = time.time() - wwl_start
            # print(f"build wwl_k time: {build_time}")
