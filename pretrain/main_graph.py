...
def graph_classification_evaluation(model, pooler, dataloader, num_classes, lr_f, weight_decay_f, max_epoch_f, device, mute=False):
    ...
    ((test_f1_mean, test_f1_std), (test_acc_mean, test_acc_std)) = evaluate_graph_embeddings_using_svm(x, y)
    print(f"#Test_acc: {test_acc_mean:.4f}±{test_acc_std:.4f}")
    return ((test_f1_mean, test_f1_std), (test_acc_mean, test_acc_std))
...
def evaluate_graph_embeddings_using_svm(embeddings, labels):
    ...
    test_f1_mean, test_f1_std = np.mean(f1_scores), np.std(f1_scores)
    test_acc_mean, test_acc_std = np.mean(accuracy_scores), np.std(accuracy_scores)
...
def main(args):
    ...
    for i, seed in enumerate(seeds):
        ...
        model.eval()
        (test_acc_mean, _),(test_f1_mean, _) = graph_classification_evaluation(model, pooler, eval_loader, num_classes, lr_f, weight_decay_f, max_epoch_f, device, mute=False)
        acc_list.append(test_acc_mean)
        f1_list.append(test_f1_mean)
    final_acc, final_acc_std = np.mean(acc_list), np.std(acc_list)
    final_f1, final_f1_std = np.mean(f1_list), np.std(f1_list)
    print(f"# final_acc: {final_acc:.4f}±{final_acc_std:.4f}")
    save_as_json({
        'acc':[final_acc, final_acc_std],
        'f1': [final_f1, final_f1_std], 
        }, f"./classifications/GraphMAE/{dataset_name}_graph_5folds.json")
