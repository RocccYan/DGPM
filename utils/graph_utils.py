...
#     def temp(edge, spotlights, level, graphs, x_base, batch, ):
#         u, v = edge
...
#         node_features = np.stack(list(map(lambda n: 
#             x_base[n].cpu().numpy(), sorted(list(spotlight)))))
#         return (subgraph, node_features)
    
#     subgraphs_X = list(map(lambda edge: 
#         temp(edge, spotlights, level, graphs, x_base, batch,), edge_index.T))
#     subgraphs, X = list(zip(*subgraphs_X))
...
def get_edge_subgraphs(edge_index, spotlights, level, graphs, x_base, batch, hop=False):
    """Return one spotlight subgraph per edge.
    """
    # logging.info(f"get edge subgraph: Before spotlight {time.time()}")
    spotlights_new = [spotlights[level][u.item()].union(spotlights[level][v.item()]) 
        for u,v in edge_index.T]

    # logging.info(f"get edge subgraph: Before subgraph {time.time()}")
    subgraphs = [graphs[batch[list(spotlight)[0]]].subgraph(spotlight).copy() 
        for spotlight in spotlights_new]

    # logging.info(f"get edge subgraph: Before X {time.time()}")
    X = [np.stack(list(map(lambda n: x_base[n].cpu().numpy(), sorted(list(spotlight))))) 
        for spotlight in spotlights_new]

    # logging.info(f"get edge subgraph: Before return {time.time()}")
    return subgraphs, X
